...
import { ScrollFadeIn, StaggerList, StaggerItem } from "../components/ui/FadeIn";
import {
  CalendarDays,
  UserCircle,
  Link2,
      ...
      <section className="-mx-8 px-8 py-24 bg-slate-50">
        ...
        <div className="hidden md:grid grid-cols-[1fr_40px_1fr_40px_1fr] gap-x-2">
          {steps.flatMap(({ number, icon: Icon, title, description }, i) => {
            const card = (
              <ScrollFadeIn key={number} delay={i * 0.12}>
        ...
        <div className="md:hidden flex flex-col">
          {steps.map(({ number, icon: Icon, title, description }, i) => (
            <ScrollFadeIn key={number} delay={i * 0.1}>
        ...
        <StaggerList className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {features.map(({ icon: Icon, title, description }) => (
            <StaggerItem key={title}>
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 group h-full">
